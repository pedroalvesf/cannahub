import { Body, Controller, ForbiddenException, HttpCode, NotFoundException, Param, Patch, UseGuards } from '@nestjs/common';
...
@RequirePermission('association_catalog', 'update')
export class AssociationUpdateProductController {
  constructor(
  ...
  async handle(
    ...
    if (result.isRight()) {
      return { product: { id: result.value.product.id.toString() } };
    }
  }
}
